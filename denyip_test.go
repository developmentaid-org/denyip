...
func TestNewDenyIP(t *testing.T) {
	testCases := []struct {
		...
		{
			desc:     "valid ipv6",
			denyList: []string{"2001:db8::1"},
		},
		{
			desc:     "valid ipv6 range",
			denyList: []string{"2001:db8::/32"},
		},
	}
...
func TestNewDenyIP_ServeHTTP(t *testing.T) {
	testCases := []struct {
		...
		{
			desc:       "allowed ipv6 with remote address",
			denyList:   []string{"2001:db8::/32"},
			remoteAddr: "[2001:db8:1::1]:1234",
			xff:        "",
			expected:   403,
		},
		{
			desc:       "denied ipv6 with remote address",
			denyList:   []string{"2001:db8::1"},
		...
		{
			desc:       "denied ipv6 range with remote address",
			denyList:   []string{"2001:db8::/32"},
			remoteAddr: "[2001:db8:1::1]:1234",
			xff:        "",
			expected:   403,
		...
		{
			desc:       "allowed ipv6 with x-forwarded-for",
			denyList:   []string{"2001:db8::/32"},
			remoteAddr: "",
			xff:        "2001:db8:2::1, 2001:db8:3::1",
			expected:   403,
		},
		{
			desc:       "denied ipv6 with x-forwarded-for",
			denyList:   []string{"2001:db8::/32"},
			remoteAddr: "",
			xff:        "2001:db8:1::1, 2001:db8:2::1",
			expected:   403,
		},
		{
			desc:       "allowed ipv6 with remote address outside deny range",
			denyList:   []string{"2001:db8::/48"},
			remoteAddr: "[2001:db8:1:1::1]:1234",
			xff:        "",
			expected:   200,
		},
		{
			desc:       "allowed ipv6 with x-forwarded-for outside deny range",
			denyList:   []string{"2001:db8::/48"},
			remoteAddr: "",
			xff:        "2001:db8:2:1::1, 2001:db8:3:1::1",
			expected:   200,
		},
	}
